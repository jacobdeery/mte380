...
double IcpPointToPoint::fitStep(double *T, const int32_t T_num, Matrix &R, Matrix &t,
...
std::vector<int32_t> IcpPointToPoint::getInliers(double *T, const int32_t T_num, const Matrix &R,
    ...
    if (dim == 2) {
        ...
        for (int32_t i = 0; i < T_num; i++) {
            ...
            if (neighbor[0].dis < indist) inliers.push_back(i);
        }
