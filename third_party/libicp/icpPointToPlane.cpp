...
// Also see (3d part): "Linear Least-Squares Optimization for Point-to-Plane ICP Surface
// Registration" (Kok-Lim Low)
double IcpPointToPlane::fitStep(double *T, const int32_t T_num, Matrix &R, Matrix &t,
...
#pragma omp parallel for private(i) default(none) \
    shared(T, active, nact, p_m, p_t, A, b, r00, r01, r10, r11, t0, t1)  // schedule (dynamic,2)
        for (i = 0; i < nact; i++) {
        ...
        // establish correspondences
#pragma omp parallel for private(i) default(none)                                                \
    shared(T, active, nact, p_m, p_t, A, b, r00, r01, r02, r10, r11, r12, r20, r21, r22, t0, t1, \
           t2)  // schedule (dynamic,2)
        for (i = 0; i < nact; i++) {
            // kd tree query + result
            std::vector<float> query(dim);
            kdtree::KDTreeResultVector result;
            ...
            // transform point according to R|t
            query[0] =
                (float)(r00 * T[idx * 3 + 0] + r01 * T[idx * 3 + 1] + r02 * T[idx * 3 + 2] + t0);
            query[1] =
                (float)(r10 * T[idx * 3 + 0] + r11 * T[idx * 3 + 1] + r12 * T[idx * 3 + 2] + t1);
            query[2] =
                (float)(r20 * T[idx * 3 + 0] + r21 * T[idx * 3 + 1] + r22 * T[idx * 3 + 2] + t2);
...
std::vector<int32_t> IcpPointToPlane::getInliers(double *T, const int32_t T_num, const Matrix &R,
    ...
    if (dim == 2) {
        ...
        for (int32_t i = 0; i < T_num; i++) {
            ...
            if ((sx - dx) * nx + (sy - dy) * ny < indist) inliers.push_back(i);
        }
        ...
        for (int32_t i = 0; i < T_num; i++) {
            ...
            // check if it is an inlier
            if ((sx - dx) * nx + (sy - dy) * ny + (sz - dz) * nz < indist) inliers.push_back(i);
        }
    }
